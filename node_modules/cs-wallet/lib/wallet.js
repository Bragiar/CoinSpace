...
function Wallet(options) {
    ...
    discoverAddresses(this.api, this.externalAccount, this.internalAccount, addressFunction,
        function(err, addresses, changeAddresses, balance, unspentAddresses, txIds) {
            ...
            fetchUnspents(that.api, unspentAddresses, function(err, utxos) {
                ...
                that.unspents = utxos;
                //this.traceabilityUnspents = getTraceabilityUnspents(historyTxs, utxos);
                //done(null, that) // Veit ekki hvað þetta gerir en þetta lætur mig geta sótt historytx í pages
                // SKOÐA BETUR
    ...
    //this.traceabilityUnspents = getTraceabilityUnspents();
    //console.log(this.traceabilityUnspents)
...
Wallet.prototype.gethistoryTxs = function() {
    console.log(this.historyTxs)
...
Wallet.prototype.createTx = function(to, value, data, fee, minConf, unspents) {
    ...
    console.log("unspents before sort")
    console.log(utxos)
    if (unspents != null) { //IF traceability transaction, leave the first unspent in index 0 to use it
      const trace_unspent = utxos.splice(0,1);
      utxos = utxos.sort(function(o1, o2) {
          return o2.value - o1.value
      })
      utxos.splice(0,0,trace_unspent[0]);
    ...
    console.log("unspents after sort")
    console.log(utxos)
...
Wallet.prototype.createTxWithData = function(to, value, data, fee, minConf, unspents) {
    if (typeof value === 'string') value = parseInt(value);
    if (typeof fee === 'string') fee = parseInt(fee);
...
Wallet.prototype.getUnspents = function(){
...
Wallet.prototype.serialize = function () {
    ...
    return JSON.stringify({
        ...
        traceabilityBalance: this.getTraceabilityBalance(),
        unspents: this.unspents,
        historyTxs: this.historyTxs,
        minConf: this.minConf,
        traceabilityUnspents: this.traceabilityUnspents
...
Wallet.deserialize = function(json) {
    ...
    wallet.traceabilityBalance = deserialized.traceabilityBalance,
    wallet.unspents = deserialized.unspents
    wallet.historyTxs = deserialized.historyTxs
    wallet.minConf = deserialized.minConf
    wallet.traceabilityUnspents = deserialized.traceabilityUnspents
